...
export const updateImageExpression = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updatedExpression = await expressionService.updateImage(IDExpression, urlImage);

    return successResponse(res, "Expression image updated successfully", updatedExpression);
  } catch (error) {
