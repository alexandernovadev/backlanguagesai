...
export const getExamsWithAttempts = async (
  ...
  try {
    ...
    // Filters
    const level = req.query.level as string | string[];
    ...
    const exams = await examService.getExamsWithAttempts(userId, {
      page,
      limit,
      level,
      language,
    ...
    return successResponse(res, "Exams with attempt information retrieved successfully", exams);
  } catch (error) {
    return errorResponse(
      res,
      "An error occurred while retrieving exams with attempt information",
      500,
      error
    );
  }
